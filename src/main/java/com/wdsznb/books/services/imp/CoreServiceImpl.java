...
import com.wdsznb.books.Handler.LogHandler;
import com.wdsznb.books.Handler.MsgHandler;
import com.wdsznb.books.Handler.ScanHandler;
import com.wdsznb.books.Handler.SubscribeHandler;
...
public class CoreServiceImpl implements CoreService {
    ...
    public void refreshRouter() {
        final WxMpMessageRouter newRouter = new WxMpMessageRouter(
                this.wxMpService);
        // 记录所有事件的日志
//    有BUG 暂且放下    newRouter.rule().handler(this.logHandler).next();
        // 关注事件
        newRouter.rule().async(false).msgType(WxConsts.XML_MSG_EVENT)
                .event(WxConsts.EVT_SUBSCRIBE).handler(this.subscribeHandler)
                .end();
//        扫描事件---想用这个获取到openID
        newRouter.rule().async(false).msgType(WxConsts.QRCONNECT_SCOPE_SNSAPI_LOGIN).event(WxConsts.XML_MSG_EVENT).handler(this.scanHandler);
        // 默认,转发消息给客服人员
